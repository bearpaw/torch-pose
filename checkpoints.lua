...
function checkpoint.saveMultiModels(epoch, model, optimState, isBestModel, opt)
   local modelSave = {}
   for k, v in pairs(model) do
      v:clearState()
      if torch.type(v) == 'nn.DataParallelTable' then
         modelSave[k] = v:get(1)
      else
         modelSave[k] = v
      end
   ...
   if isBestModel then
      torch.save(paths.concat(opt.save, opt.expID, 'model_best.t7'), model)
   end
end
