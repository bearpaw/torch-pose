...
function Trainer:train(epoch, dataloader)
   ...
   local N = 0
   local iterSize = self.opt.iterSize
   local nIter = 0

   print('=> Itersize = ' .. iterSize)
   ...
   for n, sample in dataloader:run() do
      ...
      if sample ~= nil then
         ...
         if self.isDebug then
            ...
            local outIm = visualizer:drawOutput(self.input[1][{{1,3},{},{}}]:float(), self.model.output[#self.model.output][1]:float())
...
function Trainer:test(epoch, dataloader)
   ...
   for n, sample in dataloader:run(false) do
      ...
      --    if sampleAcc[ii] < 0.5 then 
      --       local outIm = visualizer:drawOutput(self.input[ii][{{1,3},{},{}}]:add(0.5):float(), output[#output][ii]:float(), (preds_hm[ii]-0.5)*5)
      --       image.save(('hardcase/mpii_hg_s8_r1_v3_bn_preact_useconv2/valid_%.4d.png'):format(sample.index[ii]), outIm)
      --    end
      -- end
...
function Trainer:multiScaleTest(epoch, dataloader, scales)
   ...
   for n, sample in dataloader:run(false, scales) do
      ...
      -- Compute accuracies
      local acc, preds_hm = self:computeScore(output, self.target)
      lossSum = lossSum + loss*batchSize
      accSum = accSum + acc*batchSize
      N = N + batchSize
...
function Trainer:copyInputs(sample)
   ...
   if torch.type(self.target) == 'table' then
      ...
      for a = 2, self.opt.nAttention do
          table.insert(self.target, self.target[1])
      end
   else
...
function Trainer:computeScore(output, target)
   ...
   local function heatmapAccuracy(output, label, idxs, outputRes)
      ...
      -- prune gt label 
      local gtMask = label:view(label:size(1), label:size(2), label:size(3)*label:size(4)):max(3):gt(0.5):repeatTensor(1, 1, 2)
      gt:cmul(gtMask:float())
