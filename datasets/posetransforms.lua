...
-- Draw mask 
-------------------------------------------------------------------------------
function M.drawMask(img, pt, meanstd, hsize, wsize)
    -- Draw a 2D gaussian
    -- Check that any part of the gaussian is in-bounds
    assert(meanstd ~= nil, 'Meanstd cannot be nil')
    local hsize = hsize or 10
    local wsize = wsize or 10
    -- perturb pt
    pt = pt + torch.Tensor(2):uniform(0, 10)
    -- crop
    local ul = {math.floor(pt[1] - hsize), math.floor(pt[2] - hsize)}
    local br = {math.floor(pt[1] + wsize), math.floor(pt[2] + wsize)}
    -- If not, return the image as is
    ...
    for c = 1, 3 do img:sub(c, c, img_y[1], img_y[2], img_x[1], img_x[2]):fill(meanstd.mean[c]) end
    return img
end
