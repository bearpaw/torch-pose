...
function MpiiDataset:augmentation(input, label)
  ...
  if self.split == 'train' then
      ...
      if self.mask > 0 then
         local tpts = self.tpts
         local nParts = tpts:size(1)
         local maskProb = torch.Tensor(nParts):uniform() 
         for i = 1,nParts do
            if tpts[i][1] > 0 and maskProb[i] <= self.mask then -- Checks that there is a ground truth annotation
               local hsize = math.floor(torch.uniform(2, 40))
               local wsize = math.floor(torch.uniform(2, 40))
               drawMask(input, tpts[i], self.meanstd, hsize, wsize)
            end
         end
